...
- 常驻“练习工作台”可直接打开当前练习、练习档案和热题 100；本地 CRUD、切换、导出和力扣操作不会调用模型。
...
点击“切换到该练习”后，插件直接更新本地会话并打开工作台中的当前练习，通过本地 Toast 确认结果，不会额外唤醒 Agent。用户下一次真正发送消息时，工具会从当前会话游标读取已经切换的完整练习上下文。
...
任意自然语言 CRUD 仍需要模型理解意图；如果希望完全避免模型费用，应使用工作台按钮或上述本地命令。删除操作始终保留 UI 二次确认，不提供快捷删除命令。
