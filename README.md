...
开始前必须明确提供以下五项配置，插件和 Agent 不会自行推断或使用默认值：
...
简历出题模式还必须提供简历原文。缺少配置时，Agent 只能按“模式、主题、难度、题数、是否追问”的顺序询问缺少项，不得重复询问或增加其他配置。

可以一次性描述完整配置：

```text
开始 JVM 八股练习，中级，共 10 题，不追问
开始 Redis 高级场景题练习，共 8 题，根据回答追问
开始 Java 高级后端模拟面试，共 12 题，根据回答追问，主题是并发和系统设计
根据下面这份简历进行高级简历面试，共 10 题，根据回答追问：……
```
