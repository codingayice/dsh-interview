...
- “进行中”支持新建和管理未结束练习；“练习档案”只展示已结束练习，并支持查看、重新打开、删除和导出。
- 能力复盘提供平均分、主题掌握度和薄弱主题。
- 聊天卡片与右侧轻量题目时间轴使用同一份权威读模型；点击节点后通过题目、作答记录、答案三个标签查看详情。
- 常驻“练习工作台”按生命周期分为进行中、练习档案和热题 100；本地 CRUD、切换、导出和力扣操作不会调用模型。
...
结束非力扣练习后，Agent 会读取本次练习的全部题目、历次作答、评价和讲解，生成并保存总体总结、表现亮点和改进建议。力扣练习会直接在本地结束，只汇总本次抽取的题目，不生成能力分析。
...
“进行中”只显示尚未结束的练习，可新建、切换和继续；“练习档案”只显示已经结束的练习。两者都支持搜索和模式筛选，并展示每道题的历次回答、评价和讲解。Markdown 导出通过一次性受控令牌下载。

点击“切换到该练习”后，插件直接更新本地会话，题目时间轴随即表示当前选择，并通过本地 Toast 确认结果，不会额外唤醒 Agent。已结束练习必须先“重新打开”，恢复为进行中后才能继续。

练习与会话是一对一绑定：新会话默认不选择任何练习，也不会显示题目时间轴；只有新建、切换或重新打开练习才会建立绑定。同一练习被另一个会话切换后，完整进度会转移到新会话，旧会话立即失去选择；结束练习后绑定自动释放，但练习内容仍完整保存在练习档案中。

### 零模型费用操作

以下操作通过 `UI → InterviewCoordinator → InterviewApplication → SQLite` 在本地完成：

- 查看、搜索和筛选进行中练习与练习档案
- 切换练习、打开历史题和重新作答
- 修改或删除练习与题目
- 导出 Markdown
- 新建力扣练习、随机下一题、查看热题 100 和标记完成状态

只有生成普通面试题、评价回答、生成点评讲解和生成练习总结会创建显式 `agentTasks` 并调用模型。

任意自然语言 CRUD 仍需要模型理解意图；如果希望完全避免模型费用，应直接使用工作台 UI。删除操作始终保留 UI 二次确认。
